		...
		// hide upon page load
			...
			function minuteTimer(callback) {
				var timerFunc = function () {
					...
					var now = 600000 * Math.floor(Date.now() / 600000 + 1.0);
					// run the callback
					callback(now);
			...
			setTimeout(function () {
                ...
                if(msgCount === 0){
					...
					if(!customMsg.classList.contains("blink")){
						...
						if(window.navigator.userAgent.includes("Mozilla") && !window.navigator.userAgent.includes("Chrome")){
							//console.log("firefox: engaged static marquee");
							marquee.stop();
							setTimeout(function () {
								marquee.start();
							}, 3000);
		...
		function msg_submit() {
			    // Check for empty input.
		            if(input.value === "" || username.value === "") {
		                ...
		                // fade out over 3 seconds after 5 seconds
		                setTimeout(function () {
		                	error.classList.toggle("fadeOut");
		                	setTimeout(function () {
		                		error.innerText = "";
